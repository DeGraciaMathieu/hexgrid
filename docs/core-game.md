...
Phase de mouvement (optionnelle) : déplace une seule unité de sa couleur.
Phase de résolution de combat : on examine toutes les captures déclenchées par le mouvement.
...
En cas de conflit (unité ennemie posée sur une ville située dans ton territoire) : l'occupation physique trumphe le territoire. La ville est contrôlée par le joueur dont une unité est dessus.
Scoring des villes
